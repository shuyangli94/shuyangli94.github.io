...
function drawMaze(mazeGrid, clear=false) {
	context.clearRect(0, 0, dagCanvas.width, dagCanvas.height);
...
async function createMaze(ruleset) {
	...
	nCol = dims.cols * 1;
	nRow = dims.rows * 1;
	dagCanvas.width = nCol * scaling;
	dagCanvas.height = nRow * scaling;
	dagCanvas.style = null;
	...
	// Initialize game
	dagCanvas.addEventListener("keydown", checkKey, true);
	initializePlayer();
}